...
export const getFireBot = (uuid, convId) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      ref.child(uuid).orderByChild('access').once('value', (data) => {
        db.goOffline()
        app.delete()
        if (data.exists()) {
          for (var key in data.val()) {
            if (data.val().hasOwnProperty(key)) {
              if (data.val()[key] && data.val()[key].access && data.val()[key].access.length) {
                for (var i = 0; i < data.val()[key].access.length; i++) {
                  if (data.val()[key].access[i] && data.val()[key].access[i] === convId) {
